...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//tablero de la mesa
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(3.0f, 0.1f, 2.0f)); // se escala en x y z para hacer la mesa (tabla) ancho, grosor y profundidad
		model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));// trasladamos en Y (Se subio)
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		//cubo encima de la mesa
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f)); // se escala en x y z para hacer la mesa (tabla) ancho, grosor y profundidad
		model = glm::translate(model, glm::vec3(0.0f, 0.6f, 0.0f));// trasladamos en Y (Se subio)
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		//asiento 
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(0.8f, 0.1f, 0.8f)); // se escala en x y z para hacer la mesa (tabla) ancho, grosor y profundidad
		...
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(0.1f, 0.9f, 0.8f)); // se escala en x y z para hacer la mesa (tabla) ancho, grosor y profundidad
...
void Inputs(GLFWwindow* window) {
	if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)  //GLFW_RELEASE, si usa escape sale del programa
